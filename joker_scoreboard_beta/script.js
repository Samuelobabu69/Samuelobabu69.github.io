$(document).ready(() => {
    ...
    async function init() {
        //sessionStorage.removeItem("translations-update-website-beta")
        ...
        let response = await fetch(
            "https://raw.githubusercontent.com/Samuelobabu69/joker-scoreboard/refs/heads/main/beta/version.json",
            { cache: 'no-store' })
            .then(data => data.json())
    ...
    async function getTranslation(id) {
        if (!sessionStorage.getItem("translations-update-website-beta")) {
            const response = await fetch('assets/translations.json');
            let translations = await response.json();
            sessionStorage.setItem("translations-update-website-beta", JSON.stringify(translations));
        }
        let translations = await JSON.parse(sessionStorage.getItem("translations-update-website-beta"));
